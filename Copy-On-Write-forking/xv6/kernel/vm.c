...
faulthandler(struct proc* p)
{
  ...
  if(va > USERTOP){
    panic("handler: Invalid virtual address");
    p->killed = 1;
    return;
  }
  pde = &(proc->pgdir[PDX(va)]); // get page directory entry
  if(*pde & PTE_P){
    pte_t *pgtab = (pte_t*)(PTE_ADDR(*pde)); // get page table
    pte = &(pgtab[PTX(va)]); // get page table entry
    if(!(*pte & PTE_P)){
      panic("faulthandler: page not present");
    }
    pa = PTE_ADDR(*pte);
  }else{
    panic("faulthandler: pde should exist");
  }
  if(!(*pte & PTE_W)){
    ...
    if(refcount == 1){
      ...
      *pte = (*pte | PTE_W);
      ...
      lcr3(PADDR(pgdir));
      memmove(mem, (char*)pa, PGSIZE);
      *pte = (uint)mem | flags; //may have to change
      lcr3(PADDR(pgdir));
      return;
    }
...
pde_t*
walkpgdirhelper(pde_t *pgdir, const void *va, int create)
{
